Gem::Specification.new do |s|
  s.name        = 'datarepos_finder_base'
  s.version     = '0.1'
  s.date        = '2016-01-11'
  s.summary     = "DataRepos finder infrastructure"
  s.description = "The most primitive errors required for other base gem packages of DataRepos"
  s.authors     = ["Bill McKinnon"]
  s.email       = 'bmck@datarepos.xyz'
  s.files       = ["lib/datarepos_finder_base.rb"]
  s.homepage    = 'http://www.datarepos.xyz'
  s.add_dependency 'spidr'
  s.add_dependency 'datarepos_base_errors'
end
